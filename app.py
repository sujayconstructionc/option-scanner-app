# app.py
# Auto 3:30 PM Baseline + Live F&O Option Scanner
...
st.set_page_config(page_title="Auto F&O Option Scanner", layout="wide")
st.title("⚡ Auto 3:30 PM Baseline + Live F&O Scanner")
st.caption("Runs live, captures baseline at 3:30 PM automatically. CE/PE filter applied.")
...
with st.sidebar:
    ...
    top_n_main = st.number_input("Top N Volume Spike", min_value=5, max_value=200, value=50, step=5)
...
def capture_baseline():
    for sym in FO_SYMBOLS[:scan_limit]:
        ...
        rows = rec.get("data", [])
        if not rows or underlying is None: continue
...
# ---------------- Automatic 3:30 PM baseline ----------------
now=datetime.now()
if now.time() >= dt_time(15,30) and not st.session_state.get("auto_baseline_done", False):
    capture_baseline()
    st.session_state.auto_baseline_done=True
    st.success("✅ Automatic 3:30 PM baseline captured!")
    ...
    if run_main:
        ...
        for sym in FO_SYMBOLS[:scan_limit]:
            ...
            for r in rows:
                ...
                for t in ['CE','PE']:
                    if t in r and r[t]:
                        if (t=='CE' and not show_ce) or (t=='PE' and not show_pe): continue
                        d=r[t]
...
with col2:
    ...
    if not st.session_state.baseline:
        st.info("Capture baseline first or wait till 3:30 PM.")
    else:
        run_gainers=st.button("▶ Run Premium Gainers")
        if run_gainers:
            gainers=[]
            now_ts=datetime.now().strftime("%H:%M:%S")
            for sym in FO_SYMBOLS[:scan_limit]:
...
st.markdown("---")
st.caption("Auto 3:30 PM baseline capture | Live NSE F&O Scanner | CE/PE filter applied | Runs 9:15–15:30 IST")
